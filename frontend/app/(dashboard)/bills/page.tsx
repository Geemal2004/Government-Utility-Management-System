...
export default function BillsPage() {
  ...
  const fetchBills = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/v1/bills?${queryParams.toString()}`
      );
      ...
      const result: PaginatedResponse<Bill> = await response.json();
  ...
  const fetchSummary = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/bills/summary`);
